// page fade
$(window).on('load', function () {
  scroll_effect();
  $(window).scroll(function () {
    scroll_effect();
    $('html').removeClass('effect-fade');
  });
});
function scroll_effect() {
  $('.effect-fade').each(function () {
    var elemPos = $(this).offset().top;
    var scroll = $(window).scrollTop();
    var windowHeight = $(window).height();
    if (scroll > elemPos - windowHeight) {
      $(this).addClass('effect-scroll');
    }
  });
}


// header animation
$(window).on('load scroll', function () {
  var scrollPos = $(this).scrollTop();
  if (scrollPos > 47) {
    $('header').addClass('scrolled');
  } else {
    $('header').removeClass('scrolled');
  }
});


// pagetop & smooth scroll
$('a[href^="#"]').click(function () {
  var offset = $(window).width() <= 640 ? 60 : 50;
  var speed = 500;
  var href = $(this).attr("href");
  var target = $(href == "#" || href == "" ? 'html' : href);
  var position = target.offset().top - offset;
  $("html, body").animate({ scrollTop: position }, speed, "swing");
  return false;
});


// top slider
$(function () {
  var sld_wrap = '#slider',
    sld = '.p-index-kv-img__item',
    sld_length = $(sld).length,
    pager = '.p-index-kv-pager__item',
    sld_img_wrap = '.sld-img-wrap',
    sld_pre = 'sld',
    pager_pre = 'pager',
    sld_wait = 6000,
    sld_time = 1300,
    sld_timer;

  $('.sld-img').each(function (index, element) {
    var img_url = $(element).data('img');
    $(element).css('background-image', 'url("' + img_url + '")');
  });

  $(sld).first().children(sld_img_wrap).addClass('initloading');

  $.fn.slide_fade = function () {
    return this.each(function (i, elem) {
      var sldnum = parseInt($(this).data('sldnum'));
      var sldnum_next = sldnum < sld_length ? (sldnum + 1) : 1;

      $(sld).not('#' + sld_pre + sldnum).removeClass('is-current');
      $('#' + sld_pre + sldnum).addClass('is-current');

      $(sld).not('#' + sld_pre + sldnum_next).removeClass('is-next');
      $('#' + sld_pre + sldnum_next).addClass('is-next');

      $(pager).not('#' + pager_pre + sldnum).removeClass('is-current');
      $('#' + pager_pre + sldnum).addClass('is-current');

      $(pager).addClass('now-sliding');

      setTimeout(function () {
        $(pager).removeClass('now-sliding');
      },
      sld_time);

      clearTimeout(sld_timer);
      sld_timer = setTimeout(function () {
        sldnum++;
        if (sldnum > sld_length) {
          sldnum = 1;
        }
        $(sld_wrap).data('sldnum', sldnum).slide_fade();

        $(sld).first().children(sld_img_wrap).removeClass('initloading');
      },
      sld_wait);
    });
  };

  $.fn.slide_select = function () {
    return this.each(function (i, elem) {
      var num = parseInt($(this).data('selectrnum'));
      var num_next = num < sld_length ? (num + 1) : 1;

      $('#' + sld_pre + num).removeClass('is-next');

      $(sld).not('#' + sld_pre + num).removeClass('is-select');
      $('#' + sld_pre + num).addClass('is-select');

      $(pager).not('#' + pager_pre + num).removeClass('is-current');
      $('#' + pager_pre + num).addClass('is-current');

      $(pager).addClass('now-sliding');

      setTimeout(function () {
        $(sld).not('#' + sld_pre + num).removeClass('is-current');
        $('#' + sld_pre + num).addClass('is-current');

        $(sld).not('#' + sld_pre + num_next).removeClass('is-next');
        $('#' + sld_pre + num_next).addClass('is-next');

        $(pager).removeClass('now-sliding');

        $(sld).first().children(sld_img_wrap).removeClass('initloading');
      },
      sld_time);

      sldnum = num;
      clearTimeout(sld_timer);
      sld_timer = setTimeout(function () {
        $('#' + sld_pre + num).removeClass('is-select');

        sldnum++;
        if (sldnum > sld_length) {
          sldnum = 1;
        }
        $(sld_wrap).data('sldnum', sldnum).slide_fade();
      },
      sld_wait);
    });
  };

  $(sld_wrap).data('sldnum', 1).slide_fade();

  $('.p-index-kv-pager__item').on('click', function () {
    var selectrnum = $(this).attr('id').slice(-1);
    $(sld_wrap).data('selectrnum', selectrnum).slide_select();
  });
});


// FAQ accordion
$('#p-faq input').change(function () {

  $(this).each(function () {
    if ($(this).is(':checked')) {
      $(this).nextAll('.block-accordion').slideDown();
    }
    else {
      $(this).nextAll('.block-accordion').slideUp();
    }
  })
})


function sortMenu($selectElem, dataName, targetName) {
  var cateSelected = $selectElem.data(dataName);

  $selectElem.siblings().removeClass('current');
  $selectElem.addClass('current');

  $(targetName).fadeOut(200, function () {
    $(targetName).find('article').each(function () {
      var cateData = $(this).data(dataName);

      if ((cateData.match(cateSelected)) || (cateSelected == 'all')) {
        $(this).show();
      } else {
        $(this).hide();
      }
    });
    $(targetName).fadeIn(1000);
  });
}

// WORKS sort
$("#p-works-view .sort-menu li").on("click", function () {
  sortMenu($(this), 'category', '#works-view');
});

// SERVICE sort
$(".sort-menu li").on("click", function () {
  sortMenu($(this), 'category', '#service-select');
});

// Price sort
$(".sortprice-menu li").on("click", function () {
  sortMenu($(this), 'price', '#price-select');
});

$(function () {
  var tableClone;
  $('.price-table-web, .price-table-movie').each(function(i, elem) {
    tableClone = $(elem).clone();
    tableClone.addClass('clone');
    $(elem).after(tableClone);
  })
})


// SERVICE Photo
$(function () {
  var $target = $('.sv-photo');

  if ($target[0]) {
    var $grid = $target.masonry({
      itemSelector: 'none', // select none at first
      columnWidth: '.grid-sizer',
      gutter: '.gutter-sizer',
      percentPosition: true,
      transitionDuration: '1.2s',
      visibleStyle: { opacity: 1 },
      hiddenStyle: { opacity: 0 },
    });

    var msnry = $grid.data('masonry');

    $grid.imagesLoaded(function () {
      $grid.masonry('option', { itemSelector: '.grid-item' });
      var $items = $grid.find('.grid-item');
      $grid.masonry('appended', $items);
    });

    $grid.infiniteScroll({
      path: 'photo-{{#}}.html',
      append: '.grid-item',
      outlayer: msnry,
      scrollThreshold: 100,
      history: false,
    });

    $grid.on('append.infiniteScroll', function (event, response, path, items) {
      galleryPhoto();
    });
  }
});


// Gallety Photo
function galleryPhoto() {
  var $lg = $('#gallery-photo');

  if ($lg[0]) {
    if ($lg.data('lightGallery') != null) {
      $lg.data('lightGallery').destroy(true);
    }

    $lg.lightGallery({
      addClass: 'lg-outer-photo',
      mode: 'lg-fade',
      cssEasing: 'ease-in',
      speed: 800,
      width: '91%',
      startClass: '',
      backdropDuration: 800,
      controls: false,
      preload: 2,
      selector: '.grid-item',
      download: false,
      counter: false,
      enableDrag: false,
      thumbnail: false,
      autoplayControls: false,
      fullScreen: false,
      zoom: false,
      share: false,
      hash: true,
      galleryId: 1,
    });

    $lg.on('onSlideItemLoad.lg', function (event) {
      var y = $(window).scrollTop();
      $(window).scrollTop(y + 1);
      $('.lg').addClass('lg-open');
    });

    $lg.on('onAfterAppendSubHtml.lg', function (event) {
      $('.lgc-close').on('click', function () {
        $lg.data('lightGallery').destroy();
      });
      $('.lgc-prev').on('click', function () {
        $lg.data('lightGallery').goToPrevSlide();
      });
      $('.lgc-next').on('click', function () {
        $lg.data('lightGallery').goToNextSlide();
      });
    });
  }
}
$(function () {
  galleryPhoto();
});


// Gallety Movie
$(function () {
  var $lg = $('#gallery-movie');

  if ($lg[0]) {

    $lg.lightGallery({
      addClass: 'lg-outer-movie',
      mode: 'lg-fade',
      cssEasing: 'ease-in',
      speed: 800,
      startClass: '',
      backdropDuration: 800,
      // closable: false,
      controls: false,
      preload: 2,
      download: false,
      counter: false,
      enableDrag: false,
      thumbnail: false,
      autoplay: false,
      autoplayControls: false,
      autoplayFirstVideo: false,
      videoMaxWidth: '91%',
      youtubePlayerParams: { modestbranding: 1, rel: 0 },
      vimeoPlayerParams: { byline: 0, portrait: 0, title: false },
      fullScreen: false,
      zoom: false,
      share: false,
      hash: true,
      galleryId: 2,
    });

    $lg.on('onSlideItemLoad.lg', function (event) {
      var y = $(window).scrollTop();
      $(window).scrollTop(y + 1);
    });

    $lg.on('onAfterAppendSubHtml.lg', function (event) {
      $('.lgc-close').on('click', function () {
        $lg.data('lightGallery').destroy();
      });
      $('.lgc-prev').on('click', function () {
        $lg.data('lightGallery').goToPrevSlide();
      });
      $('.lgc-next').on('click', function () {
        $lg.data('lightGallery').goToNextSlide();
      });
    });
  }
});
